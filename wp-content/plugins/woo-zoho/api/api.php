...
$json['invoices']='["reference_number","place_of_supply","gst_treatment","gst_no","template_id","date","payment_terms","payment_terms_label","due_date","discount","tax_total","shipping_charge","is_discount_before_tax","discount_type","is_inclusive_tax","exchange_rate","recurring_invoice_id","invoiced_estimate_id","salesperson_name","project_id","allow_partial_payments","notes","terms","adjustment","adjustment_description","reason","tax_authority_id","tax_exemption_id","invoice_number","tax_id","tax_treatment","vat_treatment","branch_id"]'; //recurringinvoices
 ...
 $json['customerpayments']='["payment_mode","amount","date","reference_number","description","exchange_rate","bank_charges","account_id","tax_account_id","branch_id"]';
 $json['items']='["name","sku","rate","description","description","unit","product_type","item_type","initial_stock","initial_stock_rate","is_taxable","tax_id","hsn_or_sac","upc","ean","isbn","part_number"]';
  
  $module_a=$module;
   if( in_array($module, array('recurringinvoices1','creditnotes'))){
       $module_a='invoices';
   }
$fields=array(); $req=array('contact_name'); $dates=array('date','start_date','end_date'); $bool=array('is_discount_before_tax','is_taxable');
...
if(!empty($arr['customfields']['customer_payment'])){
...
if(is_array($fields)){
  ...
  if(isset($fields['tax_id'])){
      $ship_tax=$fields['tax_id'];
      $ship_tax['name']='vx_ship_entry_tax';
      $ship_tax['label']='Shipping Line Tax';
...
if(!empty($post['vx_ship_entry'])){
      ...
      $ship_line=array('product'=>array('id'=>$post['vx_ship_entry']),'quantity'=>1,'list_price'=>floatval($post['shipping_charge']));
    if(!empty($post['vx_ship_entry_tax'])){
     $ship_line['Tax']=$post['vx_ship_entry_tax'];   
    }
    
 $post['Product_Details'][]= $ship_line;
}
//$extra['line items']=$post['Product_Details'];
$fields['line_items']=$post['Product_Details']; 
 if(!empty($post['vx_ship_entry'])){
...
public function push_object_invoice($module,$fields,$meta){ 
   //  $res=$this->post_crm('invoices/183210000000065031'); var_dump($res); die();
...
$module_single=rtrim($module,'s');
 ...
 $group_title='';
 if( $module == 'items' && !empty(self::$order['_product_id']) ){
...
$extra["response"]=$search_response;
      ...
      if( count($search_response)>5){
       $search_response=array_slice($search_response,-5);   
      }
      
      $extra["response"]=$search_response;
      if(isset($search_response[0][$module_single.'_id'])){
      $id=$search_response[count($search_response)-1][$module_single.'_id'];
      }
  }
}
   ...
   $unit_price=round($unit_price,2);  
    ...
    if(method_exists($_order,'get_item_total')){
     ...
     if(!empty($meta['item_price_custom'])){
      $cost=wc_get_order_item_meta($item->get_id(),$meta['item_price_custom'],true); 
     }  
       $cost=round($cost,2);
...
//var_dump($item_tax['total'],$meta['tax_map']); die();
}else{
...
$product_detail=array('price'=>$unit_price,'qty'=>$qty,'cost'=>$cost,'cost_woo'=>$cost_woo,'purchase_rate'=>'','tax'=>$tax);
if(!empty($tax_id)){
